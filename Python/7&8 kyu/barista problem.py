...
Task:
iven a list of the times you need to brew each coffee, print the minimum total waiting time and if you get it right, you will get that raise your boss promised you.
Note that:
You can only brew one coffee at a time.
ince you have one coffee machine, you have to wait for it to brew the current coffee before you can move on to the next one.
...
def barista(coffees):
    ...
    for i in range(len(zup)):
        if i ==0:
            wait_time=wait_time+ int(zup[i]) 
            sum=sum+wait_time 
        else:
            wait_time=wait_time+ int(zup[i]) +2
            sum=sum+wait_time             
